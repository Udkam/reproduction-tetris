...
describe('T27-R1 centered live stage', () => {
  ...
  it('keeps Next on the left, mode data on the right, and Mutation state below Next', () => {
    ...
    expect(hudCss).toMatch(/\.game-left-rail \.mutation-status__effect\s*\{[^}]*grid-template-columns:\s*13px minmax\(0, 1fr\) !important;[^}]*grid-template-rows:\s*auto 1px !important;/s);
