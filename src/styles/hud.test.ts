...
describe('T27-R1 centered live stage', () => {
  ...
  it('keeps Next on the left, mode data on the right, and Mutation state below Next', () => {
    ...
    expect(hudCss).toMatch(/\.game-left-rail \.mutation-status__meter > i\s*\{[^}]*display:\s*block !important;[^}]*height:\s*1px !important;[^}]*min-height:\s*0 !important;[^}]*padding:\s*0 !important;[^}]*border:\s*0 !important;[^}]*box-shadow:\s*none !important;/s);
