...
describe('Phase 9 run-result authority', () => {
  ...
  it('keeps two meaningful metric cards in a narrow portrait ledger with reduced motion', () => {
    expect(result).toMatch(/\.action-sheet--run-result\s*\{[\s\S]*width:\s*min\(31rem, calc\(100vw - 24px\)\)/);
    expect(result).toMatch(/\.run-result__metrics\s*\{[\s\S]*grid-template-columns:\s*minmax\(0, 1fr\)/);
