...
const COPY: Record<AppLanguage, Translation> = {
  'zh-CN': {
    ...
    rules: {
      ...
      race: ['开局｜带 3 层上升基岩。', '压力｜13 秒逐步缩短至 6 秒；每消 3 行移除一层基岩。', '结束｜基岩把堆叠顶到顶端。'],
      sprint: ['目标｜像经典一样清行；每累计 6 行，下落提速一级。', '载具｜特殊整块任一格被清除时，立即释放一次道具。', '效果｜冻结停落；坍缩列内下沉；炸弹清空底部 3 行；加倍让消行得分 ×2。相同效果再次触发会额外增加 10 秒；加倍再次触发升级为超级加倍 ×4。', '结束｜方块堆到顶端。'],
    ...
    phrasing: {
      ...
      leaderboardSummary: (score, pieces, lines, survival, mutation) => mutation ? `${score} 分  ${pieces} 方块` : survival ? `${lines} 行  ${pieces} 方块` : `${score} 分`,
  ...
  en: {
    ...
    phrasing: {
      ...
      leaderboardSummary: (score, pieces, lines, survival, mutation) => mutation ? `${score} pts  ${pieces} pieces` : survival ? `${lines} lines  ${pieces} pieces` : `${score} pts`,
