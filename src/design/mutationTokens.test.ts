...
describe('mutation VFX token contract', () => {
  it('keeps the four live visual families distinct and within the renderer budget', () => {
    ...
    expect(Object.keys(MUTATION_VFX_TOKENS)).toEqual(['freeze', 'collapse', 'bomb', 'multiplier']);
  ...
  it('defines the required readable bomb and timed-state durations without changing Core ticks', () => {
    ...
    expect(MUTATION_VFX_TOKENS.collapse.audio).toEqual({
      activateHz: 148,
      accentHz: 93,
      waveform: 'triangle',
      gain: 0.13,
    });
    expect(MUTATION_VFX_TOKENS.multiplier.audio).not.toHaveProperty('loopHz');
    expect(MUTATION_VFX_TOKENS.multiplier.audio).not.toHaveProperty('endHz');
  });
