...
| 解谜只是一张单层面板 | 六条以上可滚动 flat rows 展示 `关卡 N/总数`、代表性名称/难度、固定序列与清空目标；局内有 restart/back 和剩余方块 |
...
- 每案断言页面无横纵溢出、关键 rect 均在 viewport、模块无重叠、board 比率 `2 ± .02`、一个 Next、五个 controls ≥44px、完整模式名、禁止 Hold/暂存/旧设计词/过期 `40 行`、零 console error。
- 每案断言 topbar 与 `Tetris` 标题的完整 DOMRect 在 viewport 内、computed overflow 为 `visible`；desktop、portrait 与 844 × 390 landscape 的 essential text 都至少 12px。
- paused 断言外部 `暂停` 为 0、纸带在 board 内且高度 ≤18%；mode switch 断言 board 无文字且有 `应用并重新开始` / `返回本局`。
- 六关 list 额外断言至少六条扁平行、selected `关卡 3/6`，并把滚动容器推进到底证明第六关可达；所有可见文本无 clip。
