...
| 6. Classic | coordinator as `t15_classic_writer`; ordinary-clear source is frozen and the second slice owns only Classic Renderer cues | `t15_classic_rules_qa` + `t15_classic_visual_qa` run serially after candidate freeze | contract `7aed5ef`; ordinary clear `1a163ff`; baseline audit/docs `1ba6c26..`; two-file feedback source; evidence; QA; acceptance | `FEEDBACK CONTRACT / OPEN`; base `4f871ac`; Core, UI/CSS/audio, Puzzle data/selector and other-mode effects frozen |
