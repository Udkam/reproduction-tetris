...
| 3. HUD | coordinator as `t15_hud_writer` | `t15_phase3_input_qa` + `t15_phase3_rules_qa` + `t15_phase3_visual_qa` | DOM/input/a11y, spawn presentation, final HUD CSS, and compact-English correction remain separately revertible | `ACCEPTED LOCAL`; source `741d8a6`, all final audits ACCEPT, recovery push pending |
...
| 5. Mutation | sequential `t15_mutation_core_writer`, `t15_mutation_vfx_writer`, `t15_mutation_ui_writer` | `t15_mutation_rules_qa` + `t15_mutation_visual_qa` + `t15_mutation_evidence_qa` | Core/performance, renderer/VFX, and UI each remain independently revertible | `OPEN`; Ice 1 s/cell and carrier-recognition addendum frozen |
