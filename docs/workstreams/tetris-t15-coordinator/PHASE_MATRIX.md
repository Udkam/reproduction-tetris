...
| 5. Mutation | sequential `t15_mutation_core_writer`, `t15_mutation_vfx_writer`, `t15_mutation_ui_writer`, then bounded `t15_mutation_evidence_writer` | `t15_mutation_rules_qa` + `t15_mutation_visual_qa` + `t15_mutation_evidence_qa` | Core RNG/Ice, Core Collapse/performance, runtime FIFO, renderer/VFX, UI, telemetry, harness, gate logs, raw browser frames, browser index, each QA verdict, corrections, and acceptance remain separate rollback nodes | `PAUSED-EVIDENCE-OPEN`; rollback `fd7ef8d`; product `f6fa06e`; gates `96a3841` PASS (26/224, 753 modules); harness correction `3d01e9f` static PASS; overloaded browser runs published no artifacts; resume with managed browser evidence, three QA verdicts, acceptance, cleanup and non-force push |
