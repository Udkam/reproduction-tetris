...
| 1.5 Modal compositor closure | coordinator as `t15_modal_writer` | `t15_modal_rules_preaudit` + `t15_modal_target` | one narrow CSS/test source checkpoint, then evidence/QA | `PREAUDIT PASS`; rollback `dfeb2c9`, first-entry zero-canvas contract corrected |
