...
| 5. Mutation | sequential `t15_mutation_core_writer`, `t15_mutation_vfx_writer`, `t15_mutation_ui_writer`, then bounded `t15_mutation_evidence_writer` | `t15_mutation_rules_qa` + `t15_mutation_visual_qa` + `t15_mutation_evidence_qa` | Core RNG/Ice, Core Collapse/performance, runtime FIFO, renderer/VFX, UI, and read-only evidence observability remain independently revertible | `EVIDENCE-REPAIR-OPEN`; rollback `fd7ef8d`; Core accepted at `f2d51ca`; Renderer static accepted at `69730a1`; UI semantics accepted through `65ffd19`; responsive `d819d92` static PASS; preflight rejected port-only attribution/FIFO-blind capture, so bounded renderer telemetry + managed-server harness repair remains |
