...
| 5. Mutation | sequential `t15_mutation_core_writer`, `t15_mutation_vfx_writer`, `t15_mutation_ui_writer`, then bounded `t15_mutation_evidence_writer` | `t15_mutation_rules_qa` + `t15_mutation_visual_qa` + `t15_mutation_evidence_qa` | Core RNG/Ice, Core Collapse/performance, runtime FIFO, renderer/VFX, UI, telemetry, harness, gate logs, raw browser frames, browser index, each QA verdict, corrections, and acceptance remain separate rollback nodes | `EVIDENCE-RUN-OPEN`; rollback `fd7ef8d`; Core accepted at `f2d51ca`; Renderer static accepted at `69730a1`; UI semantics accepted through `65ffd19`; responsive `d819d92` static PASS; telemetry `f6fa06e`; harness `8c321ca` + FIFO correction `f8b31ed` static PASS; dynamic evidence/gates remain; push then PAUSE |
