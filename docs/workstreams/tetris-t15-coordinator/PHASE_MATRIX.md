...
| 6. Classic | coordinator as `t15_classic_writer`; first slice owns only shared ordinary-clear Renderer | `t15_classic_rules_qa` + `t15_classic_visual_qa` run serially after candidate freeze | contract; shared presentation helper/tests; shared Renderer/tests; later feedback slice only if baseline audit proves a gap; evidence; QA; acceptance | `CONTRACT / OPEN`; base `4f871ac`; Core, UI, Puzzle data/selector and Mutation special effects frozen |
