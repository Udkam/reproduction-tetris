...
| 5. Mutation | sequential `t15_mutation_core_writer`, `t15_mutation_vfx_writer`, `t15_mutation_ui_writer`, then bounded `t15_mutation_evidence_writer` | `t15_mutation_rules_qa` + `t15_mutation_visual_qa` + `t15_mutation_evidence_qa` | Core RNG/Ice, Core Collapse/performance, runtime FIFO, renderer/VFX, UI, telemetry, harness, gate logs, raw browser frames, browser index, each QA verdict, corrections, and acceptance remain separate rollback nodes | `PUSHED`; product `ee2aac5`, gates `6d9fc6a` PASS (26/225, 753 modules), browser raw/index `9fa98a2` / `013120a`, rules/evidence PASS with no finding, visual PASS with one retained narrow-ellipsis P3, acceptance `321ebc6`, recovery tip `4f871ac` |
| 6. Classic | coordinator; corrected product/evidence frozen | repeated rules QA ACCEPT; visual QA rejected one packet label then ACCEPT after `e247dd9`; evidence-integrity QA ACCEPT | contract `7aed5ef`; ordinary clear `1a163ff`; feedback `fee0627` / `a1f3d1b`; rejected candidate `eaed1ac`; correction `308e49c` / `9085976`; gates `2c9fd50` / `0239231` / `bee956a`; browser `4a7f95f` / `9f90ced` / `ca80416`; QA checkpoints `e190cbd` / `2034f71` / `9bfddfa` / `1a4dfda`; coordinator acceptance/recovery `d0b7406` | `ACCEPTED / PUSHED / CLOSED`; exact local/tracking/remote equality |
| 6R. Direct ordinary-clear rollback | coordinator | `t15_phase6_rules_qa` ACCEPT, P0–P3/GAP zero | clean-line source `760437d`; current-schema fixture `9730d99`; title-only correction `70cff6e`; contract/adoption `bc10882`; green gates `e2b990e`; rejected selector history preserved separately at `dce331b`; acceptance/recovery `47ee3e5` | `ACCEPTED / PUSHED / CLOSED`; exact local/tracking/remote equality, selector product paths unchanged from prior recovery |
| 7. Puzzle 50 | coordinator as the sole writer for sequential v5, five ten-level batches, unlock, then existing-selector adaptation | `t15_puzzle_rules_qa` + `t15_puzzle_visual_qa` + `t15_puzzle_evidence_qa` | contract `08c0491`, persistence-v5 `fbec049`, five accepted ten-level batches through `e8bc42b`, progress-unlock acceptance/recovery `62ba8e9`; selector visual work is excluded from the current goal | `CURRICULUM / PERSISTENCE / PROGRESS ACCEPTED AND PUSHED`; selector visual adaptation deferred |
| 8. Integration | coordinator | `t15_final_rules_qa` + `t15_final_visual_qa` + `t15_final_evidence_qa`; tie-break if needed | contract `bb38413`; harness `8044068` / `667cb54` / `aa1afe6`; raw/index `94463de` / `a599f4c`; rules/visual/evidence QA `e7fc800` / `78c0664` / `7f45db5`; coordinator acceptance `c77790b`; final docs-only recovery record follows as a separate checkpoint | `ACCEPTED / PUSHED / CLOSED`; frozen product `4e4cca1`, gates PASS (26/235, 753 modules), all three QA ACCEPT, one retained narrow-ellipsis P3, selector redesign excluded |
