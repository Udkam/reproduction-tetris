...
| 5. Mutation | sequential `t15_mutation_core_writer`, `t15_mutation_vfx_writer`, `t15_mutation_ui_writer` | `t15_mutation_rules_qa` + `t15_mutation_visual_qa` + `t15_mutation_evidence_qa` | Core/performance, renderer/VFX, and UI each remain independently revertible | `CONTRACT`; rollback `fd7ef8d`, three-way baseline audit next |
