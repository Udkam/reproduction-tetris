...
| 6. Classic | coordinator as correction writer in only Renderer + direct test | first `t15_classic_rules_qa` returned one P2; repeat rules, then visual/evidence QA strictly serial | contract `7aed5ef`; ordinary clear `1a163ff`; feedback contract `fee0627`; cues `a1f3d1b`; first candidate `eaed1ac`; first gates/browser `50e3693` / `d7fb4fa`; overhang correction; fresh gates/evidence; repeated QA; acceptance | `RULES QA REJECT / P2 CORRECTION OPEN`; landing must freeze true support cells; all other product/evidence paths closed |
