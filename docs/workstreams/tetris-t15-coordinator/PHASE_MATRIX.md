...
| 6. Classic | coordinator as `t15_classic_writer`; all product source is frozen; bounded evidence writer is now open | `t15_classic_rules_qa` + `t15_classic_visual_qa` run serially after evidence freeze | contract `7aed5ef`; ordinary clear `1a163ff`; feedback contract `fee0627`; cues `a1f3d1b`; landing correction/final source `eaed1ac`; gates `55e5a7b` / `fb9ccc2` / `50e3693`; harness; raw/index; QA; acceptance | `GATES PASS / EVIDENCE WRITER OPEN`; candidate `eaed1ac` over base `4f871ac`; Core, UI/CSS/audio, Puzzle data/selector and other-mode effects frozen |
