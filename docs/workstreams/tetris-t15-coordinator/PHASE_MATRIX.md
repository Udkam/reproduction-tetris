...
| 5. Mutation | sequential `t15_mutation_core_writer`, `t15_mutation_vfx_writer`, `t15_mutation_ui_writer`, then bounded `t15_mutation_evidence_writer` | `t15_mutation_rules_qa` + `t15_mutation_visual_qa` + `t15_mutation_evidence_qa` | Core RNG/Ice, Core Collapse/performance, runtime FIFO, renderer/VFX, UI, telemetry, harness, gate logs, raw browser frames, browser index, each QA verdict, corrections, and acceptance remain separate rollback nodes | `EVIDENCE-CORRECTION`; rollback `fd7ef8d`; product `f6fa06e`; gates `96a3841` PASS (26/224, 753 modules); `74a67fe` guards capture startup and `bfc3198` decouples FIFO from PNG latency; fresh QA requires same-label elapsed reset, actual-column Collapse capture, four reduced-motion activations, then rerun/QA/acceptance/cleanup/non-force push |
