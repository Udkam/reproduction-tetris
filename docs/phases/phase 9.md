...
**CONTRACT / SOURCE NOT STARTED（2026-07-31）。**
...
2. 生存 Renderer：同族但可区分的基岩/落石和局部洞穴压力。
3. 共享微反馈：短促落底压印与普通消行缝线。
...
- 保留清楚格线；用暖棕矿物族、固定坐标变体、层理、裂纹和缺角表现洞穴。
- 基岩暗且压实；可消落石亮且新鲜；双石共用外轮廓和中缝。
...
- 普通消行最多 9 ticks；按中间到两侧激活每格内部的材质缝线；主体原位、
  原尺寸、可辨；粒子为零。
- reduced motion 仍保留静态接触与同时出现的十格缝线淡出。
