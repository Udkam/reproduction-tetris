...
const output = path.resolve('docs/evidence/t36/browser');
...
const waitForServer = async () => {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    try {
      ...
      // The single exact-owned preview process is still starting.
    }
...
try {
  ...
  await page.addInitScript(() => {
    ...
    const contexts = [];
    window.__t36AudioContexts = contexts;
  ...
  await page.screenshot({ path: path.join(output, 'mutation-gameplay.png'), fullPage: true });

  const heading = page.locator('.game-left-rail .mutation-status__header > strong');
  await heading.waitFor({ state: 'visible' });
  const visual = await page.evaluate(() => {
    const panel = document.querySelector('.game-left-rail .mutation-status.mutation-status--vfx');
    const title = document.querySelector('.game-left-rail .mutation-status__header > strong');
    const board = document.querySelector('[data-testid="board-frame"]');
    const panelBefore = panel ? getComputedStyle(panel, '::before') : null;
    const titleStyle = title ? getComputedStyle(title) : null;
    const boardRect = board?.getBoundingClientRect();
    return {
      title: title?.textContent?.trim() ?? null,
      titleTextShadow: titleStyle?.textShadow ?? null,
      panelBeforeContent: panelBefore?.content ?? null,
      panelBeforeBackgroundImage: panelBefore?.backgroundImage ?? null,
      boardRect: boardRect
        ? { width: boardRect.width, height: boardRect.height, x: boardRect.x, y: boardRect.y }
        : null,
    };
  });
  ...
  const runtime = await page.evaluate(() => ({
    ...
    audioContexts: window.__t36AudioContexts.map((context) => context.state),
  ...
  if (visual.titleTextShadow !== 'none') failures.push(`Mutation heading still glows: ${visual.titleTextShadow}`);
  if (visual.panelBeforeContent !== 'none' || visual.panelBeforeBackgroundImage !== 'none') {
    failures.push(`Mutation panel pseudo-element still glows: ${JSON.stringify(visual)}`);
  }
  if (!visual.boardRect || visual.boardRect.width < 300 || visual.boardRect.height < 500) {
    failures.push(`Board geometry mismatch: ${JSON.stringify(visual.boardRect)}`);
  }
