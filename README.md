...
> **v6 进行中（2.5D 立体化）**：引擎已加入格子高度（z）与三种 3D 机制——**斜坡 / 索桥（只承人）/ 升降台**，外加正式化的**竖直折跃**；并新增独立的等轴 **2.5D 渲染器**（纯 CSS transforms，无 Three.js）真实表现高低差/推下/升台。**正式 3D 章节已开始上线**：「立体·入门」「立体·架桥升降」两章共 8 关（菜单内与 Classic 章分隔）。旧 52 个 Classic 关仍走原 2D 网格、行为不变（共 60 关）。**双端受力**目前仅完成规则预研（详见 `claude.md` §9.4），未上线。另已补「继续上次 / 下一关」体验闭环。详见 `claude.md` §9。
