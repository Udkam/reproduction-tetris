...
- Player review rejects the accepted T28 mix as globally too soft. T29 freezes a new
  audio-only hierarchy: concise controls, physical state/hazard cues, and strongly
...
- Survival ordinary gravity is now 38 ticks per cell, and every fourth natural
  pressure rise deterministically adds two rows with an advance HUD warning and
  restart/replay/hash coverage.
- Mutation adds the seeded instantaneous Reshape carrier, which rewrites the first
  post-activation Next entry to `I`; Supergravity and Multiplier use restrained,
  constant-alpha persistent fields instead of rain strokes or a central seal.
- The sole persisted language control is back in Settings while Home keeps the four
  permanent English mode names. Source `4c43619` passes final typecheck, `305 passed /
  3 skipped` tests, and the 762-module build. Evidence `e8418d5` proves the resolved
  Aftershock and activation-free fields with zero browser errors and full cleanup;
  independent read-only QA reports P0-P3 zero.

## T22 accepted — 2026-08-02

- Removed the visible `语言 / Language` prefix from the sole Home language selector
  while preserving localized accessible names, persistence, and keyboard behavior.
- English prose and interface roles now use Playwrite NZ Basic first; the TetraMorph
  wordmark keeps its existing stroke-weighted brand treatment, while Chinese and
  numeric-data families remain unchanged.
- Product `0a0f2bc` passes direct tests `49/49`, final typecheck, the full suite
  (`300 passed / 3 skipped`), and the 762-module build. Evidence `631393a` was inspected
  at original detail; independent read-only QA reports P0-P3 zero. Port 4178 has no
  listener.
...
- Frozen source `eadeac6` and evidence `b600ace` pass the visual contract, final
...
- Accepted product source `12fb0ae` and immutable browser evidence `d1656a1` close the
  nine-outcome contract, including structural no-grid basalt/boulder geometry rather
  than rock decals.
...
- Phase 10 is accepted and pushed at clean base `a2d6670`; Phase 11 is active and no
  source claim is accepted yet.
- The nine current requirements are frozen into recoverable checkpoints covering
  palette/geology, Mutation atmosphere and independent landing projection, compact
  information architecture, local bilingual typography, complete home-card edges,
...
- The Phase-C typography / Settings correction is now verified at product source `310d83a` and
  evidence `32b4457`. The final data face is Geist Mono; Settings uses one compact active
  panel; Pause preserves working Back/Settings hit targets and a visible Next. Final
  typecheck, `309 passed / 3 skipped`, the 768-module build, and nine-state bilingual
